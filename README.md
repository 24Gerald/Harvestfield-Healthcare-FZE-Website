...
| **Certified-by section** | `src/assets/partners/{iso,who,nafdac}.png`, `src/assets/certified/*-photo.jpg`, `certified` in `src/data/content.js` | Logos and photos are picked up automatically once the files exist (READMEs in each folder). Confirm each certification claim before launch. |
