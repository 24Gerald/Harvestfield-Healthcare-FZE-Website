...
| **Hero photograph** | `public/hero/` | Drop the approved image in as `hero-mother-and-child.jpg` and the hero switches to it: the 3D net is replaced by the photograph and only the mosquitoes render over it. Crop and wash are tuned by `HERO_BACKGROUND` in `src/data/siteConfig.js`. Remove the file and the 3D net comes back. |
| **3D models** | `public/models/` | Uploadable from the admin panel's **3D models** tab (GLB or glTF, up to 24 MB). Which element a model drives is set in code — see `README.txt` in that folder. The pack in *The Net* comes from `PRODUCT_MODEL` in `src/data/siteConfig.js`; remove that file and the procedural pouch returns. |
| **Social preview image** | `public/og-image.jpg` | 1200×630, rendered from the brand type and the pack still. Replace the file to change it; the URL is already wired into `index.html` and `src/lib/pageMeta.js`. |
...
| **Pack artwork (3D pack shot)** | `public/product/front.png`, `back.png` | Traced from the pack photographs by `scripts/make-product-artwork.py`. Replace with the real print artwork flattened to PNG/JPG (1200×1500, front and back), then re-render the still with `?packpose=-0.32`. |
| **Pack still (loading / no-WebGL fallback)** | `public/product/pack-still.webp` | Transparent render of the 3D pack. After changing the model or artwork, re-capture it: open `/synera-duoforte?packpose=2.82` at a wide viewport, read the pack canvas back with `canvas.toDataURL('image/png')` (that pose freezes it front-on, and `preserveDrawingBuffer` is enabled only when `packpose` is set so the alpha survives), then crop to the alpha bounds and save as WebP. |
